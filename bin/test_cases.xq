...
(: ATL REPETITIONS IN CLASS ATTRIBUTES, WRONG CLASS ATTRIBUTES :)
...
declare function tc:skeleton($elements,$root)
{
   ...
   if ($element/xs:complexType) then 
     element {tc:nameElement($element)}{
         if (empty(tc:typeElement($element))) then () else (tc:typeElement($element),attribute {"attribute"}{"no"}),
         ...
         return tc:skeleton(tc:elementsSequence($sequence),false()) 
         )
...
return
        ...
        modify(
        replace node 
        $new//*[tc:path-to-node-with-pos(.)=$type] with element {name($pos)} { 
          $value,$pos/*,$pos/text()}
        )
        return $new
        else
        ...
        copy $new := $tree
          ...
          return  replace node $parent  with element {name($parent)} 
          {attribute {name($pos)} {$value}, $parent/@*, $parent/*,$parent/text() }
...
let $structure := tc:skeleton($sch/xs:schema/xs:element,true())
